...
// TODO INITIALISATION ET APPEL AUTH
        ...
        links: [
          {
            ...
            href: '/project',
            desc: 'Parcourez l’ensemble du catalogue',
...
<template>
  <nav class="relative w-full">
    ...
    <div class="md:hidden flex items-center justify-between w-full p-4">
      ...
      <button class="text-white p-2 focus:outline-none" @click="isMobileMenuOpen = !isMobileMenuOpen">
        <UIcon v-if="!isMobileMenuOpen" class="w-6 h-6" name="i-heroicons-bars-3-20-solid"/>
        <UIcon v-else class="w-6 h-6" name="i-heroicons-x-mark-20-solid"/>
      </button>
    </div>
      ...
      >
        ...
        <template v-if="item.sections && item.sections.length">
          ...
          >
            ...
            <UIcon :class="openIndex === idx ? 'rotate-180' : ''" class="w-4 h-4 transition-transform"
                    ...
                    <ul class="space-y-2">
                      <li v-for="(link, lIdx) in section.links" :key="lIdx">
                        <NuxtLink :to="link.href"
                                  class="group flex items-start gap-3 rounded-md p-2 hover:bg-gray-50 transition-colors">
                          ...
                          <UIcon class="w-4 h-4 text-gray-300 group-hover:text-primary"
